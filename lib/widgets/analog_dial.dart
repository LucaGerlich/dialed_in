...
class AnalogDial extends StatefulWidget {
  ...
  const AnalogDial({
    ...
    this.max = 20,
    this.step =
        0.1, // Default step, but only for graphical representation and default for logic if none provided by provider
    required this.value,
...
class _AnalogDialState extends State<AnalogDial> {
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    ...
    if ((newValue / effectiveStep).floor() !=
        (_currentValue / effectiveStep).floor()) {
      // Check if crossed a major tick (whole number) or minor tick (0.5)
      final oldFloor = _currentValue.floor();
      final newFloor = newValue.floor();

      if (oldFloor != newFloor) {
        // Crossed a whole number - heavy haptic
        HapticFeedback.heavyImpact();
      } else {
        // Crossed a half step - light haptic
...
class _RadioTunerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = alignedStart; i <= endVal; i += step) {
      ...
      final opacity = (1.0 - (yOffset.abs() / (size.height / 2))).clamp(
        0.0,
        1.0,
      );
      ...
      if (isMajor) {
        textPainter.text = TextSpan(
          text: roundedI.toStringAsFixed(
            (step - step.roundToDouble()).abs() < 0.0001 ? 0 : 1,
          ),
          style: TextStyle(
            fontFamily: 'RobotoMono',
            fontSize: 14,
        ...
        textPainter.paint(
          canvas,
          Offset(
            dx - tickLength - textPainter.width - 8,
